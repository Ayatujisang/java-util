...
/**
 * 操作Cookie的工具类，可以设置httpOnly属性，即js无法操作的cookie，防止xss注入。
 */
public class CookieUtil {
    ...
    static {
        for (int i = 0; i < specialChars.length(); i++) {
            checkFlag[specialChars.charAt(i)] = true;
        }
    }
    ...
    private static void setCookie(String key, String value, int maxAge, String path, String domainName, boolean httpOnly, HttpServletResponse response) {
        setCookie(key, value, maxAge, path, domainName, httpOnly, false, response);
    }

    private static void setCookie(String key, String value, int maxAge, String path, String domainName, boolean httpOnly, boolean secure, HttpServletResponse response) {
        if (response != null) {
