...
public class CommonUtil {
    ...
    public static <E extends Id> Map<Integer, E> mapById(List<E> list) {
        Map<Integer, E> map = Maps.newHashMap();
        for (E e : list) {
            int id = e.getId();
            map.put(id, e);
        }
    ...
    public static <K, E> List<K> pick(List<E> list, Function<E, K> functionK) {
        List<K> returnList = Lists.newArrayList();
        for (E e : list) {
            returnList.add(functionK.apply(e));
        }
        return returnList;
    }
}
