...
public class LocalCache {
    ...
    // notFoundCallback：key找不到后 执行的业务逻辑
    public Object getValue(String key, NotFoundCallback notFoundCallback) {
        CacheEntity cacheEntity = cache.get(key);
        Object result = null;

        if (cacheEntity != null && !checkTimeExpire(key, cacheEntity)) {
            result = cacheEntity.getValue();
        }

        if (result == null) {
            result = notFoundCallback.onNotFound();
        }
